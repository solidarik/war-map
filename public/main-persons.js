...
import addPersons from './addPersons.js';
...
//////////////////////////////////////////////////
function startApp() {
    const persons = window.state.persons
    const person = window.state.person

    var personsTable = new addPersons("persons-table", persons);
    personsTable.clearTable();

    if (persons && persons.length > 0) {
      personsTable.fillTable();
      $("#persons-table tr:eq(0) td:first-child span").click();
    } else {
      $('#persons-info-container').hide()
      personsTable.showItem(person)
    }
}
