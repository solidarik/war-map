...
function updateDasboardFunction(h, handle, label, xScale) {
  ...
  label.attr("x", xScale(h)).text(listYearDasboard[h2]); //console.log("updateDasboardFunction");

  var fIndicatorData = filterIndicatorData(listYearDasboard[h2]); //console.log("fIndicatorData = " + JSON.stringify(fIndicatorData));
...
function filterIndicatorData(year) {
  var fdataGDPTOP10 = dataGDPTOP10.filter(function (n) {
  ...
  fdataGDPTOP10.forEach(function (d) {
    d.forEach(function (d1) {
      var obj = {};
      var indObj = {}; //console.log(year+" "+d1.iso3);
    ...
    d3.json(urlAgriculture, function (error, dAgriculture) {
      ...
      d3.json(urlIndustry, function (error, dIndustry) {
        ...
        d3.json(urlServices, function (error, dServices) {
          ...
          var fIndicatorData = filterIndicatorData(listYearDasboard[0].toString()); //console.log("fIndicatorData = " + JSON.stringify(fIndicatorData));
...
function dashboard(id, fData) {
  ...
  fData.forEach(function (d) {
    d.total = d.gdp
    /*d.indicators.agriculture+d.indicators.industry+d.indicators.services*/
    ;
  ...
  function histoGram(fD) {
    ...
    hGDim.w = 700 - hGDim.l - hGDim.r, hGDim.h = 300 - hGDim.t - hGDim.b; //create svg for histogram.

    var hGsvg = d3.select(id).append("svg").attr("width", hGDim.w + hGDim.l + hGDim.r).attr("height", hGDim.h + hGDim.t + hGDim.b).append("g").attr("transform", "translate(" + hGDim.l + "," + hGDim.t + ")"); // create function for x-axis mapping.

    var x = d3.scaleBand().rangeRound([0, hGDim.w]).padding(0.1).domain(fD.map(function (d) {
      return d[0];
    })); // Add x-axis to the histogram svg.

    hGsvg.append("g").attr("class", "x axis").attr("transform", "translate(0," + hGDim.h + ")").call(d3.axisBottom().scale(x)); // Create function for y-axis map.

    var y = d3.scaleLinear().rangeRound([hGDim.h, 0]).domain([0, d3.max(fD, function (d) {
      return d[1];
    ...
    .style("fill", function (d) {
      return "url(#img_" + d[2] + ")";
    }).style("stroke-width", "1px").style("stroke", "#111").on("mouseover", mouseover) // mouseover is defined below.
    ...
    function mouseover(d) {
      ...
      var st = fData.filter(function (s) {
        return s.iso3 == d[0];
          ...
          nD = d3.keys(st.indicators).map(function (s) {
        return {
          type: s,
          indicators: st.indicators[s]
        };
    ...
    hG.update = function (nD, color) {
      ...
      bars.select("rect").transition().duration(500).attr("y", function (d) {
        ...
        return hGDim.h - y(d[1]);
      }).attr("fill", color); // transition the indicatorsuency labels location and change value.
  ...
  function pieChart(pD) {
    ...
    var piesvg = d3.select(id).append("svg").attr("width", pieDim.w).attr("height", pieDim.h).append("g").attr("transform", "translate(" + pieDim.w / 2 + "," + pieDim.h / 2 + ")"); // create function to draw the arcs of the pie slices.

    var arc = d3.arc().outerRadius(pieDim.r - 10).innerRadius(0); // create a function to compute the pie slice angles.
  ...
  function legend(lD) {
    ...
    var legend = d3.select(id).append("table").attr('class', 'legend'); // create one row per segment.
    ...
    tr.append("td").attr("class", 'legendindicators').text(function (d) {
      return d3.format(",")(d.indicators);
    ...
    leg.update = function (nD) {
      ...
      var l = legend.select("tbody").selectAll("tr").data(nD); // update the indicatorsuencies.

      l.select(".legendindicators").text(function (d) {
        return d3.format(",")(d.indicators);
    ...
    function getLegend(d, aD) {
      ...
      return d3.format("%")(d.indicators / d3.sum(aD.map(function (v) {
        return v.indicators;
      })));
    }
  ...
  var tF = ['agriculture', 'industry', 'services'].map(function (d) {
    return {
      type: d,
      indicators: d3.sum(fData.map(function (t) {
        return t.indicators[d];
      }))
    };
  }); // calculate total indicatorsuency by state for all segment.

  var sF = fData.map(function (d) {
    return [d.iso3, d.total, d.iso2];
  });
  var hG = histoGram(sF),
