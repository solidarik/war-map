...
var AddMapInfoDiagramm = /*#__PURE__*/function () {
  ...
  _createClass(AddMapInfoDiagramm, [{
    ...
    value: function addMapInfoDiagrammInDiv() {
      ...
      var svg = d3.select("#" + this.divElement).append("svg").attr("id", this.divElement + "Svg").attr("width", width + margin.left + margin.right - 10).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"); //console.log("this.data=" + JSON.stringify(this.data));
      ...
      var couY = d3.max(this.data, function (d) {
        return d.rusCountry;
      });
      ...
      if (this.width >= 160 && height !== Math.round(this.height) - margin.top - margin.bottom) {
        ...
        svg.append("text").attr("x", width / 2).attr("y", -10).attr("class", "title").style("text-anchor", "middle").style("font-size", "10px").text(function () {
          if (maxY < 9999) {
            if (uniY.indexOf("тыс.") !== -1) return couY + " " + uniY.replace("тыс.", "тысячи").replace("ед", "единиц");else if (uniY.indexOf("млн") !== -1) return couY + " " + uniY.replace("млн", "миллионы").replace("ед", "единиц");else return couY + " " + uniY.replace("ед", "единиц");
          } else if (maxY < 999999) {
