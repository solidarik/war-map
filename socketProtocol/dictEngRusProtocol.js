...
class EngRusProtocol extends ServerProtocol {
                    ...
                    if (doc && typeof doc !== 'undefined') {
                        ...
                        if (isRus) {
                            ...
                            promise = new Promise((resolve, reject) => {
                                inetHelper.getEngNameFromWiki(name)
                                .then( engName => {
                                    newObj.eng = engName;
                                    resolve(newObj);
                                })
                                .catch(
                                    err => reject(`Не удалось найти англ. страницу в Википедии для ${name}: ${err}`)
                                )
                            });
                        } else {
                            newObj.eng = name;
                            promise = new Promise((resolve, reject) => {
                                inetHelper.getRusNameFromWiki(name)
                                .then( rusName => {
                                    newObj.rus = rusName;
                                    resolve(newObj);
                                })
                                .catch(
                                    err => reject(`Не удалось найти рус. страницу в Википедии для ${name}: ${err}`)
                                )
                            });
                        }
                        ...
                        promise
                        .then( newObj => {
                            log.success(`Пытаемся добавить объект ${JSON.stringify(newObj)}`);
                            return this.addEngRus(newObj.eng, newObj.rus);
                        })
                        .then( obj => resolve(obj) )
                        .catch(
                            err => reject(`Не удалось найти и добавить объект в базу: ${err}`)
                        );
                    }
                }
            )
            .catch( err => reject(`Ошибка в dictEngRusProtocol.getEndRusObject: ${err}`) );
        })
    }
    ...
    getEngRusObjectId(name) {
        return new Promise( (resolve, reject) => {
            ...
            .catch( err => reject(`Ошибка в dictEngRusProtocol.getEndRusObjectId: ${err}`) );
        });
    }
    ...
    addEngRus(eng, rus) {
        return new Promise( (resolve, reject) => {
            ...
            engRus.save((err) => {
                (err) ? reject(`Ошибка в dictEngRusProtocol.addEngRus: ${err}`) : resolve(engRus);
            });
        });
    }
